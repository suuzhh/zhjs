...
export class CacheRunner implements Runable {
  ...
  private innerGetState(task: Task): TaskState {
    ...
    if (!state) {
      state = {
        name: task.name,
        data: isFunc(task.model.default)
          ? task.model.default()
          : task.model.default,
        lastUpdated: null,
      };
  ...
  private innerRun(task: Task, currentState: TaskState): void {
    ...
    const $promise = from(task.action.run()).pipe(
      map((res) => {
        const data = task.model.dataTransfer
          ? task.model.dataTransfer(res)
          : res;
  ...
  run(task: Task): Observable<TaskState> {
    ...
    if (!this.runningTasks.has(task.name)) {
      ...
      if (
        !state.lastUpdated ||
        state.lastUpdated + this.config.interval <= Date.now()
      ) {
        innerRun(task, state);
      }
    }
